...
\]
Das Element $x$ heißt ein \defn{Fixpunkt}, falls $\Stab_G(x) = G$ ist (dies ist äquivalent zu  $g . x = x$ für alle $g \in G$.) Die Menge der Fixpunkte wird durch $\operatorname{Fix}(X)$ bezeichnet.
\end{defini}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \[
    \operatorname{Orb}_{\RR^{\times}} x = \{ \lambda. x \mid \lambda \in \RR^{\times} \} = \{ \lambda^2 x \mid \lambda \in \RR^{\times} \} = \RR_{>0} x.
    \]
    Damit ist $\operatorname{Orb}_{\RR^{\times}} 0 = \{0\}$ und für $x \neq 0$ ist $\operatorname{Orb}_{\RR^{\times}} x$ eine Halbgerade.
...
% Label for the orbit
\node[red] at (3, 2) {$\operatorname{Orb}_{\mathbb{R}^\times} 0$};
...
\end{center}
    ...
    \item $G = S_n$ operiert auf $\{1, \dots, n\}$. Für $x \in \{1, \dots, n\}$ ist \[\operatorname{Orb}_{S_n}(x) = \{1, \dots, n\},\] denn für jedes $y \in \{1, \dots, n\}$ existiert ein $\sigma \in S_n$ mit $\sigma(x) = y$. Es ist \[\Stab_{S_n}(x) = \{\sigma \in S_n \mid \sigma(x) = x\} \cong S_{n-1}.\]
    ...
    Möchte man die Linksnebenklassen als Orbits erhalten, muss man $G$ von rechts operieren lassen bzw. den "`$()^{-1}$-Trick"' anwenden: $H$ operiert von links auf $G$ durch $h . g = g h^{-1}$ (dann ist $\operatorname{Orb}_H(g) = g H$). In beiden Fällen ist $\Stab_H(g) = \{e\}$.
...
\[
G / \Stab_G(x) \to \operatorname{Orb}_G(x), \quad a \Stab_G(x) \mapsto a . x
\]
...
\begin{itemize}
    \item[(a)] Für jedes $x \in X$ ist $|\operatorname{Orb}_G(x)| = (G : \Stab_G(x))$.
...
\begin{proof}
(a) Wegen \ref{folg:isom_orbit} ist $G / \Stab_G(x) \cong \operatorname{Orb}_G(x)$,
...
\begin{align*}
\operatorname{ord}(G) &= \sum_{i=1}^n |\operatorname{Orb}_H(x_i)| = \sum_{i=1}^n |x_i H| = \sum_{i=1}^n (H : \Stab_H(x_i)) \\
...
\[
|X| = |\operatorname{Fix}_G(X)| + \sum_{i=1}^n (G : \Stab_G(x_i)).
\]
\end{folgerung}
...
\[
x\in\operatorname{Fix}_G(X) \Leftrightarrow \Stab_G(x) = G \Leftrightarrow (G : \Stab_G(x)) = 1.
\]
\end{proof}
