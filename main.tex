...
\begin{itemize}
\item Ist $\operatorname{char} K = 0$, so ist $\ker\varphi_K=0$ und daher nach dem Homomorphiesatz $\mathbb{Z} \cong \operatorname{im} \varphi_K$, also $\QQ \cong \Quot(\operatorname{im} \varphi_K) = P$.
...
$\QQ$-Algebrenhomomorphismen $\varphi_i\colon\QQ(\sqrt[4]{2})\to\QQalg$, $i=1,\ldots,4$. Diese sind eindeutig bestimmt durch das Bild von $\sqrt[4]{2}$:
\[
