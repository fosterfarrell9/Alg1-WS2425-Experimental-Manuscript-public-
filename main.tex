...
\begin{enumerate}[label=(\alph*)]
    ...
    \item $a \assoc b \iff (a) = (b) \iff $ Es existiert ein $e \in R^\times \text{ mit } a = \varepsilon b$.
\end{enumerate}
\end{bem}
...
Der Fall $r = 0$ ergibt sich direkt aus \ref{anm:primfaktorzerlegung_ist_irreduzibilitaetszerlegung}.
...
Ein Ring $R$ heißt \defn{faktoriell}, falls eine der folgenden beiden zueinander äquivalenten Aussagen gilt:
\begin{enumerate}
    \item[(i)] Jedes Element $a \in R \setminus \{0\}$ besitzt eine (wegen Satz \ref{satz:primfaktorzerlegung} automatisch bis auf RuA eindeutige) PFZ.
    \item[(ii)] Jedes Element $a \in R \setminus \{0\}$ besitzt eine bis auf RuA eindeutige IZ.
\end{enumerate}
\end{defini}
\begin{proof}
(der Äquivalenz von (i) und (ii))
...
\]
auf $\Quot(R)$ fort, der sogenannten \textbf{$p$-adischen Bewertung}. Für diese gilt für alle $x, y \in K$:
\begin{enumerate}[label=(\alph*)]
