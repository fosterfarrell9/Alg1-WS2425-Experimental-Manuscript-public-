...
\begin{satz}\label{satz:transitivitaet_galois}
(Translationssatz)  
...
\begin{proof}
(a) $\Phi$ ist wohldefiniert, denn $EF|E$ und $F|E\cap F$ sind galoissch nach \ref{anm:galoissch_ueber_zk} und die Einschränkung eines Elements $\sigma\in\Gal(EF|E)$ nach $F$ fixiert $E\cap F$. 
$\Phi$ ist injektiv,
\begin{denn}
...
\begin{itemize}
    ...
    \item[(b)] $\operatorname{Gal}(K(\mu_n)|K)$ operiert treu auf $\mu_n$ durch $\sigma.\zeta =\sigma(\zeta)$, insbesondere erhalten wir einen injektiven Gruppenhomomorphismus
    \[
    \chi \colon \operatorname{Gal}(K(\mu_n) | K) \to \Aut(\mu_n) \cong (\ZZ / n \ZZ)^\times,
    \]
    sodass für jede $n$-te Einheitswurzel $\zeta \in K(\mu_n)$ und jedes $\sigma \in \operatorname{Gal}(K(\mu_n)| K)$ gilt:
    \[
...
\[
\chi \colon \operatorname{Gal}(K(\mu_n) / K) \to S(\mu_n)
\]
mit
\[
\chi(\sigma) = 
...
Die vierten Einheitswurzeln in $\QQalg$ sind durch $1,-1,i,-i$ gegeben. Davon sind $i$ und $-i$ primitiv.
...
\begin{proof}
(a) Sei $\sigma \in \operatorname{Gal}(\QQ(\mu_n) | \QQ)$. Dann ist $\sigma|_{\mu_n}$ ein Gruppenautomorphismus von $\mu_n$. Insbesondere permutiert $\sigma$ die primitiven $n$-ten Einheitswurzeln und fixiert damit alle Koeffizienten von $\Phi_n$. 
...
\begin{itemize}
    ...
    \item[(b)] Der zyklotomische Charakter \[\chi \colon \operatorname{Gal}(\QQ(\mu_n)|\QQ) \to (\ZZ / n \ZZ)^\times\] aus \ref{satz:zyklotomischer_charakter} ist (für $K = \QQ$) ein Isomorphismus.
\end{itemize}
\end{folgerung}
\begin{proof}
...
\end{itemize}
\begin{proof}
(a) "`$\Leftarrow$"': Nach dem Satz von Lagrange gilt $p = \ord(g) \mid \ord(G)$.
...
\begin{denn}
Es gilt $\ord(G) = 30 = 2 \cdot 3 \cdot 5$. Für $p \in \{2, 3, 5\}$ gilt $s_p \mid 30$ und $s_p \equiv 1 \pmod{p}$. Somit folgt
\[
