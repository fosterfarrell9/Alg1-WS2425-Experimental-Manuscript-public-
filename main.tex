...
\paragraph{Notation.} In diesem Abschnitt sei $L|K$ stets eine algebraische Körpererweiterung.  
Es sei $\calU(L|K)$ die Menge der Untergruppen von $\Aut_K(L)$ und $\calZ(L|K)$ die Menge der Zwischenkörper von $L|K$. Sowohl auf $\calU(L|K)$ als auch auf $\calZ(L|K)$ ist durch "`$\subseteq$"' eine Ordnungsrelation gegeben.
...
$L|K$ heißt \defn{galoissch} (\defn{Galoiserweiterung}), falls $\calF_L(\Aut_K(L)) = K$ gilt.
\end{defini}
...
\[
f = (X - \sigma(\alpha)) \prod_{i=1}^n (X - \sigma(\beta_i)) = \sum_{i=0}^n \sigma(c_i) X^i \quad \text{für alle } \sigma \in H.
\]
...
$\deg f_\alpha \leq \deg \mipo_K \alpha$. Da $f(\alpha)=0$ ist und $f$ normiert ist, ist $f=\mipo_K\alpha$.
...
\begin{satz}\label{satz:galoistheorie_zerfaellt}
Die folgenden Aussagen sind äquivalent:
\begin{enumerate}
...
Nach \ref{satz:normal_aequiv} ist $L|K$ normal, und da $L$ von den Nullstellen der Polynome aus $\calF$ erzeugt wird,
ist $L|K$ separabel nach \ref{satz:separabel_eigenschaften}.
\end{proof}
