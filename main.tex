...
Seien $a, b \in R$.
\begin{itemize}
    \item $b$ heißt ein \defn{Teiler} von $a$ (Bezeichnung: $b \mid a$), falls ein 
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item $a \assoc b \iff (a) = (b) \iff $ Es existiert ein $\varepsilon \in R^\times \text{ mit } a = \varepsilon b$.
\end{enumerate}
\end{bem}

\begin{proof}
(a) Zunächst zeigen wir "`$\Rightarrow$"'.
Angenommen, $a \mid b$, d.h.\ es existiert ein $r \in R$ mit $b = r a$. Sei $x \in (b)$, d.h.\ $x = s b$ für ein $s \in R$. Dann folgt $x = s (r a) = (s r) a\in (a)$. Somit gilt $(b) \subseteq (a)$.

Für "`$\Leftarrow$"' nehmen wir an, dass $(b) \subseteq (a)$ gilt. Insbesondere ist $b \in (a)$, d.h.\ es existiert ein $r \in R$ mit $b = r a$. Daraus folgt $a \mid b$.

(b) Es gilt $a \assoc b \iff (a) = (b)$ nach Definition von $\assoc$ und (a). Zu zeigen bleibt die zweite Äquivalenz. Angenommen, $(a) = (b)$. Dann existieren $r, s \in R$ mit $b = r a$ und $a = s b$. Durch Einsetzen erhalten wir $a = s (r a) = (s r) a$, also $a (1 - r s) = 0$. Da $R$ ein Integritätsring ist, folgt $a=0$ oder $1-rs=0$.
Falls $a = 0$ ist, folgt $b = r a = 0$. Damit ist $a = 0 = 1 \cdot b$, und wir setzen $\varepsilon := 1$. Falls $1 - r s = 0$ ist, also $r s = 1$ ist, wählen wir $\varepsilon := s\in R^\times$. In beiden Fällen gilt $a = \varepsilon b$. Existiert umgekehrt ein $\varepsilon\in R^\times$ mit $a=\varepsilon b$, so folgt $b\mid a$, außerdem $b=\varepsilon^{-1}a$ und deshalb $a|b$, so dass insgesamt $a\assoc b$ folgt.
\end{proof}
...
\[
\ggT(a_1, \dots, a_n) = r_1 a_1 + \dots + r_n a_n.
\]
\end{folgerung}
...
\begin{proof}
"`(i) $\Rightarrow$ (ii)"': Seien $a, b \in R$ mit $p = a b$. Da $p$ irreduzibel ist, folgt $p \mid a$ oder $p \mid b$. Ohne Einschränkung gelte $p \mid a$, dann existiert ein $r \in R$ mit $a = r p$. Damit folgt $p = r p b$. Weil $R$ Integritätsring ist, folgt durch Kürzen $r b = 1$, also $b \in R^\times$.
...
Es gibt Hauptidealringe, die nicht euklidisch sind.
\end{anm}
...
Durch Rückwärtseinsetzen lässt sich $d$ als Linearkombination von $a$ und $b$ darstellen:
\[
d = a_n = a_{n-2} - q_{n-2} a_{n-1} = \dots = u a + v b \quad \text{mit } u, v \in R.
\]
\end{satz}
...
Angenommen, es wäre $v_p(p) > 1$. Dann gilt $p^2 \mid p$, und es existiert ein $a \in R$ mit $p = a p^2$. Daraus folgt durch Kürzen $1 = a p$ und damit $p \in R^\times$, was ein Widerspruch ist. Somit ist $v_p(p) = 1$.
...
(f) Sei $x \in R^\times$.
Angenommen, $v_p(x) > 1$. Dann gilt $p \mid x$, also existiert ein $a \in R$ mit $x = a p$. Daraus folgt $1 = x^{-1} a p$, also $p \in R^\times$, ein Widerspruch. Somit ist $v_p(x) = 0$.
...
\item Jede Einheit $a \in R^\times$ besitzt das leere Produkt als PFZ:
\[
...
Sei $a \in R \setminus \{0\}$ und $a$ besitze eine PFZ. $\PP$ sei ein Vertretersystem aller Primelemente von $R$ modulo "`$\assoc$"'. Dann gilt:
\begin{enumerate}[label=(\alph*)]
    \item Bis auf Assoziiertheit hat $a$ nur endliche viele Primteiler. 
    \item Alle PFZ von $a$ stimen bis auf Reihenfolge und Assoziiertheit überein.
    \item Jede IZ von $a$ ist eine PFZ von $a$. Insbesondere stimmen alle IZ von $a$ bis auf Reihenfolge und Assoziiertheit überein.
...
\]
eine PFZ von $a$. Wegen $a\not\in R^\times$ ist $n\geq 1$. Sei $p$ ein Primteiler von $a$. Dann existiert ein $i \in \{1, \dots, n\}$ mit $p \mid p_i$. Da $p_i$ als Primelement irreduzibel ist, folgt, dass $p \assoc p_i$ ist. Insbesondere hat $a$ bis auf Assoziertheit nur endlich viele Primteiler, womit (a) gezeigt ist. Für jedes $i\in\{1,\ldots,n\}$ existiert ein eindeutig bestimmtes Primelement in $\PP$, dass zu $p_i$ assoziiert ist. Wir nehmen im Folgenden ohne Einschränkung an, dass $p_1,\ldots,p_n$ selbst in $\PP$ liegen (andernfalls ersetzen wir sie durch die entsprechenden Assoziierten in $\PP$ und müssen ggf. die Einheit $e$ abändern).
 Sei nun
\[
a = u \cdot \prod_{i=1}^m b_i^{t_i} \tag{**} \label{eq:IZ}
\]
eine beliebige IZ von $a$. Es gilt dann $p_1\mid a$,
und da $p_1$ ein Primelement ist, teilt $p_1$ eines der Elemente $b_1,\ldots,b_m$, nach Umnummerieren etwa ohne Einschränkung $b_1$. Da $b_1$ irreduzibel ist, folgt $b_1\assoc p_1$.
Da $b_1,\ldots,b_m$ paarweise nicht assoziiert sind, ist $p_1$ zu keinem der Elemente $b_2,\ldots,b_m$ assoziiert. Dieselbe Argumentation lässt sich auch für $p_2,\ldots,p_n$
vornehmen, und wir erhalten nach Umnummerierung $b_1\assoc p_1,\ldots,b_n\assoc p_n$ und damit
insbesondere $m\geq n$. Außerdem sind die Elemente $b_1,\ldots,b_n$ selbst Primelemente,
und es ist $p_j\not\assoc b_i$ für $i\neq j$. Es ist dann für $j=1,\ldots,n$ jeweils
\[
v_{p_j}(a) = v_{p_j}\left(u \cdot \prod_{i=1}^n b_i^{t_i}\right) = v_{p_j}(u) + \sum_{i=1}^n t_i v_{p_j}(b_i)\overset{\ref{bem:p-adische_bewertung}}{=}0+\sum_{i=1}^m t_i\delta_{ij}=t_j.
\]
Da \eqref{eq:PFZ} als PFZ auch eine IZ von $a$ ist, ist $t_j=v_{p_j}(a)=w_j$ für $j=1,\ldots,n$.
Wir schreiben für $j=1,\ldots,n$ jeweils $b_j=u_jp_j$ mit $u_j\in R^\times$. Es folgt
\[
a = u \cdot \prod_{i=1}^m b_i^{t_i} = \left(u \cdot \prod_{i=1}^n u_i^{w_i}\right) \cdot \prod_{i=1}^n p_i^{w_i}\cdot\prod_{i=n+1}^m b_i^{t_i}
=\left(e^{-1}u \cdot \prod_{i=1}^n u_i^{w_i}\right) \cdot a\cdot \prod_{i=n+1}^m b_i^{t_i}.
\]
Durch Kürzen von $a$ erhalten wir, dass $\prod_{i=n+1}^m b_i^{t_i}\in R^\times$ ist. Dies hat
$m=n$ zur Folge. Die IZ \eqref{eq:IZ} ist also eine PFZ von $a$ und stimmt mit der gegebenen
PFZ \eqref{eq:PFZ} von $a$ bis auf Reihenfolge und Assoziiertheit überein. Somit stimmen je zwei
IZ und damit auch je zwei PFZ von $a$ bis auf Reihenfolge und Assoziiertheit überein, was (b) und (c) liefert. Für (d) bemerken wir
\[
a = e \cdot \prod_{i=1}^n p_i^{w_i}=e \cdot \prod_{i=1}^n v_{p_i}^{v_{p_i}(a)}=e \cdot \prod_{p \in \PP} p^{v_p(a)},
\]
wobei $v_p(a)=0$ für $p\in\PP\setminus\{p_1,\ldots,p_n\}$ ist und sich die Eindeutigkeit von $e$
aus der Annahme, dass es eine weitere Einheit gibt, welche die entsprechende Gleichung erfüllt, durch Kürzen ergibt.
\end{proof}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \]
    also gibt es zwei IZ, die nicht bis auf Reihenfolge und Assoziiertheit übereinstimmen. Insbesondere hat $2X^2$ keine PFZ in $R$ und $X, \sqrt{2} X$ sind irreduzible Elemente in $R$, die keine Primelemente sind.

    \item In $R := \ZZ[\sqrt{-3}]$ gilt:
    \[
    4 = 2 \cdot 2 = (1 + \sqrt{-3})(1 - \sqrt{-3}),
    \]
    also gibt es zwei IZ, die nicht bis auf Reihenfolge und Assoziiertheit übereinstimmen. Insbesondere hat $4$ keine PFZ in $\ZZ[\sqrt{-3}]$, und $2, 1 + \sqrt{-3}, 1 - \sqrt{-3}$ sind irreduzible Elemente in $R$, die keine Primelemente sind.
\end{enumerate}
\end{bsp}

\begin{defini}\label{def:faktorieller_ring}
Ein Ring $R$ heißt \defn{faktoriell}, falls jedes Element $a \in R \setminus \{0\}$ eine (wegen Satz \ref{satz:primfaktorzerlegung} bis auf Reihenfolge und Assoziiertheit eindeutige) PFZ besitzt.
\end{defini}

\begin{bsp}
Dass der Polynomring $\CC[X]$ faktoriell ist, lässt sich direkt aus dem Fundamentalsatz der Algebra ablesen. Dieser besagt schließlich, dass sich jedes nichtkonstante Polynom $f\in\CC[X]$
in der Form $f=e(X-a_1)\cdot\ldots\cdot(X-a_n)$ mit $e\in\CC^\times,a_1,\ldots,a_n\in\CC$ schreiben lässt, und die Polynome $X-a_i$ sind allesamt Primelemente in $\CC[X]$ (vgl. \ref{bsp:primelement_beispiele}). Wir werden gleich sehen, dass der Polynomring $K[X]$ über einem beliebigen Körper $K$ grundsätzlich faktoriell ist.
\end{bsp}
...
\]
Angenommen es wäre $(a) = (b_1)$, dann würde ein $e \in R^\times$ mit $a = e b_1 = b_1 b_2$ existieren. Dies hätte $b_2 = e\in R^\times$ zur Folge, was ein Widerspruch ist.
...
Da $R$ ein Haupidealring ist, existiert ein $a \in R$ mit $J = (a)$. Es existiert ein $i \in \NN$ mit $a \in (a_i)$, also $(a) \subseteq (a_i) \subseteq (a)$, worau $(a_i) = (a)$ und schließlich
...
\begin{bem}\label{bem:vp_diskrete_bewertung}
Seien $R$ ein faktorieller Ring, $p \in R$ ein Primel und $\PP$ ein Vertretersystem der Primelemente von $R$ modulo "`$\assoc$"'. Dann gilt für $x \in R$:
\[
v_p(x) = \infty \iff x = 0.
\]
...
\]
auf $\Quot(R)$ fort. Für diese gilt für alle $x, y \in K$:
\begin{enumerate}[label=(\alph*)]
...
Sei $R$ ein faktorieller Ring und $T \subseteq R \setminus \{0\}$ eine nichtleere Teilmenge. Dann existiert ein größter gemeinsamer Teiler (ggT) von $T$, und es gilt:
\[
...
\begin{proof}
Die Behauptung folgt recht leicht aus der Definition des ggT, \ref{anm:assoziiert_teilbarkeit} und \ref{bem:vp_diskrete_bewertung}(c).
\end{proof}
