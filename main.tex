...
\begin{proof}
(a) Die Operation ist wohldefiniert nach \ref{anm:konjugierte}. Sie ist treu, denn ist $\sigma \in \Gal(f)$ mit $\sigma(\alpha_i) = \alpha_i$ für alle $i = 1, \dots, n$, so ist $\sigma=\ident_L$ wegen $L=K(\alpha_!,\ldots,\alpha_n)$. Der Rest der Aussage folgt aus  
\ref{bem:currying}.
...
(c) "`$\Rightarrow$"' Sei $f$ irreduzibel. Dann ist $f = \mipo_K \alpha_1$.
...
\]
Es genügt folglich zu zeigen, dass $\ord(\sigma)=n$ ist. Für alle $\alpha\in\FF$ ist
\[
\alpha=\alpha^{|\FF|}=\alpha^{(p^r)^n}=\sigma^n(\alpha),
\]
also ist $\sigma^n=\ident_\FF$ und deshalb $\ord(\sigma)\leq n$.
